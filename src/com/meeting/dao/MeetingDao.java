...
public class MeetingDao {
    ...
    public int update(Meeting meeting) {
        ...
        try {
            con = DBUtils.getConnection();
            ps = con.prepareStatement("update meeting set meetingname=?,roomid=?,reservationistid=?,numberofparticipants=?,starttime=?,endtime=?,reservationtime=?,description=?,status=? where meetingid=?", PreparedStatement.RETURN_GENERATED_KEYS);
            ps.setString(1, meeting.getMeetingname());
            ps.setInt(2, meeting.getRoomid());
            ps.setInt(3, meeting.getReservationistid());
            ps.setInt(4, meeting.getNumberofparticipants());
    ...
    public int delete(int mid) {
        ...
        try {
            con = DBUtils.getConnection();
            ps = con.prepareStatement("delete from meeting where meetingid=?");
            ps.setInt(1, mid);
            return ps.executeUpdate();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
